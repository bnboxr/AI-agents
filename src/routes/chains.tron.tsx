// ── TRON Chain Route — Balance, TRX Price, SunSwap ──────────────────
...
import {
  ...
  getTronBalance,
  type TronWalletInfo,
} from "~/lib/chains/tron-wallet";
import {
  tronPaperSwap,
  getTronPaperBalances,
  resetTronPaperBalances,
  TRON_PAIRS,
  type TronSwapResult,
} from "~/lib/chains/tron-dex";

// ── Server Functions ──────────────────────────────────────────────────

const fetchTronInfo = createServerFn({ method: "GET" }).handler(
  async (): Promise<TronWalletInfo & { address: string }> => {
...
export const Route = createFileRoute("/chains/tron")({
  loader: async () => {
    try {
      const info = await fetchTronInfo();
      return { info, error: null };
    } catch (err) {
      return { info: null, error: (err as Error).message };
    }
  },
  component: TronChainPage,
});

// ── Page ──────────────────────────────────────────────────────────────

function TronChainPage() {
  const initial = Route.useLoaderData();
  const [info, setInfo] = useState(initial.info);
  const [balances, setBalances] = useState(getTronPaperBalances());
  const [swapInput, setSwapInput] = useState("TRX");
  const [swapOutput, setSwapOutput] = useState("USDT");
  const [swapAmount, setSwapAmount] = useState("100");
  const [swapping, setSwapping] = useState(false);
  const [swapResult, setSwapResult] = useState<TronSwapResult | null>(null);
  const [swapHistory, setSwapHistory] = useState<TronSwapResult[]>([]);
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const fresh = await fetchTronInfo();
        setInfo(fresh);
      } catch { /* keep stale */ }
    }, 30000);
  ...
  const handleSwap = useCallback(async () => {
    ...
    setSwapResult(null);
    try {
      const result = await tronPaperSwap(swapInput, swapOutput, amount);
  ...
  const handleReset = useCallback(() => {
    resetTronPaperBalances();
    setBalances(getTronPaperBalances());
    setSwapResult(null);
    setSwapHistory([]);
  }, []);

  const availableTokens = ["TRX", "USDT", "USDC"];
  ...
  return (
    <div className="min-h-dvh bg-darker pt-20 pb-16">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 space-y-6">
        ...
        <div className="flex items-center gap-3 animate-fade-in-up">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              SunSwap DEX · TRX Balance · Autonomous Wallet
            </p>
          </div>
          <button
        ...
        {/* Wallet Info Card */}
        {info && (
          <div className="glass-card p-5 border-l-2 border-accent-red/40 animate-fade-in-up">
            <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
              <span>🔐</span> Autonomous TRON Wallet
            </h3>
        ...
        <div className="glass-card p-5 animate-fade-in-up" style={{ animationDelay: "0.05s" }}>
          <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
            <span>🔄</span> SunSwap <span className="text-xs text-accent-yellow">(Paper Mode)</span>
          </h3>

          {/* Paper Balances */}
          <div className="grid grid-cols-3 gap-3 mb-4 text-xs">
            {(["trx", "usdt", "usdc"] as const).map((token) => (
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3 items-end">
              ...
              <button
                ...
                className="glass-button text-sm px-6 py-2 whitespace-nowrap bg-accent-red/20 border-accent-red/40 hover:bg-accent-red/30 disabled:opacity-40"
              >
                {swapping ? (
              ...
              {swapResult.success ? (
                <div className="space-y-1 text-sm">
                  ...
                  <div className="text-xs text-accent-green mt-2">
                    ✅ Paper swap executed (TRX price from CoinGecko, no real tx)
                  </div>
                </div>
              ) : (
