// ── Cosmos Chain Route — Balance, ATOM Price, Osmosis DEX ──────────
...
import {
  getCosmosAddress,
  getCosmosBalance,
  type CosmosWalletInfo,
} from "~/lib/chains/cosmos-wallet";
import {
  cosmosPaperSwap,
  getCosmosPaperBalances,
  resetCosmosPaperBalances,
  COSMOS_PAIRS,
  type CosmosSwapResult,
} from "~/lib/chains/cosmos-dex";

// ── Server Functions ──────────────────────────────────────────────────

const fetchCosmosInfo = createServerFn({ method: "GET" }).handler(
  async (): Promise<CosmosWalletInfo & { address: string }> => {
...
export const Route = createFileRoute("/chains/cosmos")({
  loader: async () => {
    try {
      const info = await fetchCosmosInfo();
      return { info, error: null };
    } catch (err) {
      return { info: null, error: (err as Error).message };
    }
  },
  component: CosmosChainPage,
});

// ── Page ──────────────────────────────────────────────────────────────

function CosmosChainPage() {
  const initial = Route.useLoaderData();
  const [info, setInfo] = useState(initial.info);
  const [balances, setBalances] = useState(getCosmosPaperBalances());
  const [swapInput, setSwapInput] = useState("ATOM");
  const [swapOutput, setSwapOutput] = useState("USDC");
  const [swapAmount, setSwapAmount] = useState("10");
  const [swapping, setSwapping] = useState(false);
  const [swapResult, setSwapResult] = useState<CosmosSwapResult | null>(null);
  const [swapHistory, setSwapHistory] = useState<CosmosSwapResult[]>([]);
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const fresh = await fetchCosmosInfo();
        setInfo(fresh);
      } catch { /* keep stale */ }
    }, 30000);
  ...
  const handleSwap = useCallback(async () => {
    ...
    setSwapResult(null);
    try {
      const result = await cosmosPaperSwap(swapInput, swapOutput, amount);
  ...
  const handleReset = useCallback(() => {
    resetCosmosPaperBalances();
    setBalances(getCosmosPaperBalances());
    setSwapResult(null);
    setSwapHistory([]);
  }, []);

  const availableTokens = ["ATOM", "USDC", "OSMO"];
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 space-y-6">
        ...
        <div className="flex items-center gap-3 animate-fade-in-up">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              Osmosis DEX · ATOM Balance · Autonomous Wallet
            </p>
          </div>
          <button
            onClick={handleRefresh}
            disabled={refreshing}
            className="ml-auto text-xs px-3 py-1.5 rounded-lg border border-dark-border text-gray-400 hover:text-white hover:border-accent-purple/40 transition-colors disabled:opacity-40"
          >
        ...
        {info && (
          <div className="glass-card p-5 border-l-2 border-accent-purple/40 animate-fade-in-up">
            <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
              <span>🔐</span> Autonomous Cosmos Wallet
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-mono text-white">
                  {info.balanceAtom.toFixed(4)} ATOM
                </p>
              </div>
              <div>
                <label className="text-xs text-gray-500 uppercase tracking-wider font-mono">
        ...
        <div className="glass-card p-5 animate-fade-in-up" style={{ animationDelay: "0.05s" }}>
          <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
            <span>🔄</span> Osmosis DEX <span className="text-xs text-accent-yellow">(Paper Mode)</span>
          </h3>

          {/* Paper Balances */}
          <div className="grid grid-cols-3 gap-3 mb-4 text-xs">
            {(["atom", "usdc", "osmo"] as const).map((token) => (
            ...
            <div className="flex gap-3 items-end">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.value === swapOutput) {
                      const others = availableTokens.filter((t) => t !== e.target.value);
                      setSwapOutput(others[0] ?? "USDC");
                    }
                  }}
              ...
              <div className="flex-1">
                ...
                <select
                  ...
                  className="glass-input w-full text-sm bg-dark-hover"
                >
                  {availableTokens.filter((t) => t !== swapInput).map((t) => (
                    <option key={t} value={t}>{t}</option>
                  ))}
                </select>
              </div>
              ...
              {swapResult.success ? (
                <div className="space-y-1 text-sm">
                  ...
                  <div className="text-xs text-accent-green mt-2">
                    ✅ Paper swap executed (ATOM price from CoinGecko, no real tx)
                  </div>
                </div>
              ) : (
